...
DECLARE
    v_count  INT;
    v_name VARCHAR2(30);
    TYPE namesarray IS VARRAY(15) OF VARCHAR2(30);
    names    namesarray;
BEGIN
    names := namesarray('students', 'students_courses', 'courses', 'majors', 'majors_courses', 'faculties', 'lecturers', 'students_courses_grades', 'grades');
...
(
    ...
    grade NUMBER (2,1) CHECK(grade IN (2, 3, 3.5, 4, 4.5, 5)),
...
(
    course_id NUMBER (4) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 CONSTRAINT course_PK PRIMARY KEY,
    name VARCHAR2 (60) NOT NULL,
    ects NUMBER (2) NOT NULL,
    lecturer_id NUMBER NOT NULL CONSTRAINT course_lecturer_fk REFERENCES lecturers(lecturer_id)
);
...
(
    ...
    student_id NUMBER NOT NULL CONSTRAINT sc_student_fk REFERENCES students(student_id),
    course_id NUMBER NOT NULL CONSTRAINT sc_course_fk REFERENCES courses(course_id),
    grade_id CONSTRAINT sc_grade_fk REFERENCES grades(grade_id)
);
